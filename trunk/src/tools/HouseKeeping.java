...
public class HouseKeeping extends MyDraggable {
  ...
  private File[] gettingTorrents() {
    ...
    File torrentDir = new File(Options._USER_DIR_ + TorrentConstants.TORRENTS_PATH);
  ...
  private File[] gettinScreenshots() {
    ArrayList<String> usedScreenshots = new ArrayList<String>();
    ArrayList<File> notUsedScreenshots = new ArrayList<File>();
    try {
      MySeriesLogger.logger.log(Level.INFO, "Getting unused screenshots");
      ResultSet r = SeriesRecord.query("SELECT screenshot FROM series");
      while(r.next()){
        String screenshot = r.getString("screenshot");
        usedScreenshots.add(screenshot);
      }
      File dir = new File(Options._USER_DIR_ + MyImagePanel.SCREENSHOTS_PATH);
      File[] files = dir.listFiles();
      for (int i = 0; i < files.length; i++) {
        File file = files[i];
        String name = file.getName();
        if (!MyUsefulFunctions.isInArray(usedScreenshots.toArray(), name)){
          notUsedScreenshots.add(file);
        }
      }
      MySeriesLogger.logger.log(Level.INFO, "Found {0} old log files", notUsedScreenshots.size());
      screenshots = notUsedScreenshots.toArray(new File[notUsedScreenshots.size()]);
      return screenshots;
